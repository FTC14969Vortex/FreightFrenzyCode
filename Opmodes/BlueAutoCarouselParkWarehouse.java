...
/**
 ...
 This file's path is: FtcRobotController/TeamCode/java/org.firstinspires.ftc.teamcode/Opmodes/BlueAutoCarouselParkWarehouse.java
 **/
...
@Autonomous(name="New Blue Carousel Warehouse Park", group="Linear Opmode")
public class BlueAutoCarouselParkWarehouse extends LinearOpMode {
        ...
        while (opModeIsActive()) {
            ...
            switch (autoStep) {
                ...
                case deliver:
                    ...
                    robot.turnRobot(-330);
                    robot.robotStop();
                    robot.alignStraight(robot.getRobotAngle(),  325, 0.1);
                    robot.robotStop();
...
//                  robot.arm.setPower(ARM_HOLDING_POWER);
                    ...
                    runtime.reset();
                    timeout_ms = 5000;
                    robot.startDriveToPosition(0.2, 60);
                    while (opModeIsActive() &&
                            (runtime.milliseconds() < timeout_ms) &&
                            (robot.FLMotor.isBusy() && robot.FRMotor.isBusy())) {
                    }
...
//                  robot.arm.setPower(ARM_HOLDING_POWER);
                ...
                case gotoCarousel:
                    runtime.reset();
                    timeout_ms = 3000;
                    robot.startDriveToPosition(0.5, 65);
                    while (opModeIsActive() &&
                            (runtime.milliseconds() < timeout_ms) &&
                            (robot.FLMotor.isBusy() && robot.FRMotor.isBusy())) {
                    }
                    ...
                    sleep(100);
                    runtime.reset();
                    robot.startStrafeToPosition(0.5, 18);
                    while (opModeIsActive() &&
                            (runtime.milliseconds() < timeout_ms) &&
                            (robot.FLMotor.isBusy() && robot.FRMotor.isBusy())) {
                    }
...
//                    sleep(100);
                    ...
                    runtime.reset();
                    robot.startDriveToPosition(0.8, -100);
                    timeout_ms = 5000;
                    while (opModeIsActive() &&
                            (runtime.milliseconds() < timeout_ms) &&
                            (robot.FLMotor.isBusy() && robot.FRMotor.isBusy())) {
                    ...
                    runtime.reset();
                    timeout_ms = 5000;
                    robot.startStrafeToPosition(0.5, 25);
                    while (opModeIsActive() &&
                            (runtime.milliseconds() < timeout_ms) &&
                            (robot.FLMotor.isBusy() && robot.FRMotor.isBusy())) {
                    }
                    ...
                    runtime.reset();
                    robot.startDriveToPosition(0.8, -150);
                    timeout_ms = 5000;
                    while (opModeIsActive() &&
                            (runtime.milliseconds() < timeout_ms) &&
                            (robot.FLMotor.isBusy() && robot.FRMotor.isBusy())) {
